...
async function deploy(contractName) {
  /* better syntax?
  return ethers.getContractFactory(contractName).then(
    async (contractFactory) => {return await contractFactory.deploy();}
  );
  */
...
async function makeComptroller(opts = {}) {
  const {
    root = await ethers.getSigner(), //saddle.account,
    kind = 'unitroller'
  } = opts || {};
...
async function makeCToken(opts = {}) {
  const {
    root = ethers.getSigner(), // saddle.account,
    kind = 'cerc20'
  } = opts || {};
...
async function setEtherBalance(cEther, balance) {
  ...
  const root = ethers.getSigner(); // saddle.account;
